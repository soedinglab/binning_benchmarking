...
def check_arguments(args):
    ...
    if result.returncode != 0:
        print(f"Error executing '{plass_executable}': {result.stderr}")
        raise RuntimeError(f"Failed to execute '{plass_executable}'.")
...
def split_binbysample(input_binfasta, path):
    ...
    for sample_id, sequences in sequences_by_sample_id.items():
        ...
        with open(os.path.join(path, output_fasta), 'w') as out_file:
...
def get_markersstat(query_sequence, path):
    subprocess.run([f'python {pyrodigal_path}/pyrodigal_prediction.py --seq {query_sequence} --outdir {path}'],\
        shell=True,
        stdout = subprocess.PIPE,
        stderr = subprocess.PIPE)
    markerfile = os.path.join(path,'marker_hits')
    if not os.stat(markerfile).st_size == 0:
        markerhits = pd.read_csv(path+'marker_hits', header=None, sep='\t')
        ...
        os.remove(os.path.join(path,'marker_hits'))
        return unique_cogs, duplicate_cogs, cont, list(markerhits[0])
    else:
        os.remove(os.path.join(path,'marker_hits'))
        return 0, 0, []

def segregate_subbins(sampleids, path, outputname, foldertype):
    # create folder for merge/non-merge bins
    secondarypath = os.path.join(path, foldertype)
    os.makedirs(secondarypath, exist_ok=True)
    if foldertype == 'merge':
        filelist = [f'{path}/{sid}_sample.fasta' for sid in sampleids]
        file_list_str = ' '.join(filelist)
        output_path = os.path.join(secondarypath, outputname)
        subprocess.run([f"cat {file_list_str} > {output_path}"], shell=True)
    else:
        for sid in sampleids:
            inputfile = f'{path}/{sid}_sample.fasta'
            outputfile = f'{secondarypath}/{sid}_sample.fasta'
            subprocess.run([f'mv {inputfile} {outputfile}'], shell=True)

def clustersubbins(samplewithscmgs, subbin_markerstat, path):
    ...
    for pair in samplepairs:
        pair1, pair2 = pair
        marker_file1 =  os.path.join(path, f'{pair1}_marker.fasta')
        marker_file2 =  os.path.join(path, f'{pair2}_marker.fasta')
        input_assembly = os.path.join(path, f'{pair1}_{pair2}_marker.fasta')
        output_assembly = os.path.join(path, f'{pair1}_{pair2}_assembly.fasta')
        subprocess.run(['cat ' + marker_file1 + ' ' + marker_file2 + ' > ' + input_assembly], shell=True)
        subprocess.run(['penguin nuclassemble ' + input_assembly + ' ' + output_assembly + ' tmp --num-iterations 1 --min-seq-id 0.995 --contig-output-mode 0 '], \
            shell=True,
            stdout = subprocess.PIPE,
            stderr = subprocess.PIPE)
        try:
        ...
        if not os.stat(output_assembly).st_size == 0:
            unique_cogs_ij, duplicate_cogs_ij, cont, _ = get_markersstat(output_assembly, path)
            ...
            print(subbin_markerstat[pair1][1], subbin_markerstat[pair2][1], 'duplicated cogs')
            print(subbin_markerstat[pair1][0], subbin_markerstat[pair2][0], 'unique cogs')
            # NOTE: condition is strict.
            if delta_duplicate_cogs == 0.0 and cont < 0.05 and merged_cogs >= 2:
...
def parallelprocess_bins(binfasta, filename, bindir, format):
    ...
    samplesinbin = split_binbysample(binfasta, path)
    subbin_markerstat = {}
    for sample_id in samplesinbin:
        query_sequence = os.path.join(path, f'{sample_id}_sample.fasta')
        unique_cogs_i, duplicate_cogs_i, cont, idslist = get_markersstat(query_sequence, path)
        if idslist:
            subbin_markerstat[sample_id] = [unique_cogs_i, duplicate_cogs_i, cont]
            samplemarker_sequence = os.path.join(path, f'{sample_id}_marker.fasta')
    ...
    clustersets = clustersubbins(samplewithscmgs, subbin_markerstat, path)
    ...
    if clustersets:
        ...
        for cluster in clustersets:
            segregate_subbins(cluster, path, f'merge_{mergecounter}.fasta', 'merge')
            mergecounter += 1
    if non_mergelist.size != 0:
        segregate_subbins(non_mergelist, path, None, 'nonmerge')
        clearpath(path)
...
def mergebins(bindir,
    ...
    if len(input_binsfastalist) == 0:
        raise RuntimeError(f'Bin folder does not contains sequence file. Check input format')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        usage="%(prog)s --bindir --format [optional] --plassdir [optional]",
        add_help=True,
    )
    ...
    mergebins(args.bindir, args.plassdir, args.format)